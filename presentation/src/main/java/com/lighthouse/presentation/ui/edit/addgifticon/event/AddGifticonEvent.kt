...
sealed class AddGifticonEvent {
    ...
    data class ShowDeleteConfirmation(val gifticon: AddGifticonItemUIModel.Gifticon) :
        AddGifticonEvent()

    data class NavigateToGallery(val list: List<GalleryUIModel.Gallery> = emptyList()) :
        AddGifticonEvent()

    data class NavigateToCrop(
        val crop: AddGifticonCrop,
        val origin: Uri,
